...
class Stuff {
  ...
  static void registerAll() {
    Contents.items.addAll([
      sticks = Item.mergeable("sticks", mass: 100).asFuel(heatValue: 5.0).tagged(["wooden", "sticks"]),
      cutGrass = Item.mergeable("cut-grass", mass: 200).asFuel(heatValue: 5.0).tagged(["straw", "flammable-floc"]),
      log = Item.mergeable("log", mass: 500).asFuel(heatValue: 60.0).tagged(["wooden", "log"]),
      ...
      ember = Item.mergeable("ember", mass: 5).asFuel(heatValue: 3.0).hasDurability(max: 100),
      tinder = Item.mergeable("tinder", mass: 5).asFuel(heatValue: 5.0).tagged(["tinder"]),
    ...
    // craft
    Contents.items.addAll([
      strawRope = Item.mergeable("straw-rope", mass: 200).asFuel(heatValue: 8.0).tagged(["rope"]),
    ]);
    // container
    Contents.items.addAll([
      plasticBottle = Item.container(
        ...
        acceptTags: ["liquid"],
        capacity: 500,
      ).asFuel(heatValue: 10.0).tagged(["plastic", "bottle"]),
      can = Item.container(
        "can",
        mass: 30,
        acceptTags: ["liquid"],
      ...
      woodenBowl = Item.container(
        ...
        mass: 200,
        acceptTags: ["liquid"],
      ).asFuel(heatValue: 15.0).hasDurability(max: 100).tagged(["wooden", "bowl"]),
    ]);
  }
}
